...
So long as the segment itself contains a consecutive exact string from the transcript. 

This allows you to:
- Remove filler words, tangents, less interesting sections, or parts not relevant to the main narrative hook
- Start with the most compelling hook, then jump to supporting content
- Keep the clip tight and engaging throughout

For each clip, in json format, provide BOTH:
1. tweet_text - The suggested tweet text that will accompany the video clip
2. segment_transcripts -list of the exact transcript of the segments that should be used to compose the clip

CRITICAL: The segment_transcripts must contain the EXACT words from the source transcript, word-for-word, including filler words and natural speech patterns, so it can be precisely matched to find timestamps in the JSON. 
Also, segment_transcripts should not include speaker labels (for example "SPEAKER A" or "SPEAKER B").
Speaker labels  just exist in the prompt you're viewing, for your readability. But they are not words in the original JSON. 
If you want to make a clip which goes over different speaker segments, which you will often need to do in order to have coherent narrative structure, just split up the parts where the speaker switches into separate segments.
...
clip_suggestion_examples = [
    {
        ...
        "segment_transcripts": [
            "Why is it the case that the Chinese stock market has performed so badly, even though the economy has grown a lot?",
            "There is a deep fundamental difference between capitalism and socialism. And it sounds very philosophical, but I think, you know, this might help sort of your Gen Z listeners think about this. Because for socialism, they only care about output. The state can use the state banking system, which they control, to allocate a huge amount of capital to maximize the output of all these different things that they care about. But when you maximize the output, you don't necessarily make money doing that. Whereas capitalism wants to maximize profit.",
...
def suggest_clips(transcript_data):
    ...
    full_prompt = clip_suggestions_prompt.format(
...
def segment_transcript_to_timestamps(segment_transcript, transcript_data):
    ...
    if best_ratio < 0.7:
        print(f"WARNING: Low confidence match ({best_ratio:.2%})")
        return None, None
